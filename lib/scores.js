(function () {
  ...
  function save(apples, seconds, result) {
    try {
      var list = getAll();
      list.push({ apples: apples, seconds: seconds, result: result,
  ...
  function render() {
    ...
    var rows = list.map(function (row, i) {
      ...
      return '<tr class="' + cls + '">' +
        '<td class="sb-num">'    + (i + 1)          + '</td>' +
        '<td class="sb-apples">' + row.apples        + '</td>' +
        '<td class="sb-time">'   + fmt(row.seconds)  + '</td>' +
        '<td class="sb-result">' + res               + '</td>' +
        '<td class="sb-date">'   + row.date          + '</td>' +
      '</tr>';
    }).join('');
    el.innerHTML =
      '<p class="sb-title">High Scores</p>' +
