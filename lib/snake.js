(function () {
  ...
  var Snake = (window.SnakeGame.Snake = function (level) {
    ...
    this.xDim = level.cols;
    this.yDim = level.rows;
    this.level = level;
    this.direction = 'up';
    var startX = Math.floor(this.xDim / 2);
    var startY = Math.floor(this.yDim / 2);
    this.joints = [
      [startX, startY + 2],
      [startX, startY + 1],
      [startX, startY],
  ...
  Snake.prototype.isValidJoint = function (position) {
    return (
      position[0] >= 0 &&
      position[0] < this.xDim &&
      position[1] >= 0 &&
      position[1] < this.yDim &&
    ...
    this.joints.forEach(
      function (joint, idx) {
        ...
        if (isHead) {
          ...
          grad.addColorStop(
            0.5,
            'rgba(' + baseR + ',' + baseG + ',' + baseB + ',' + alpha.toFixed(2) + ')'
          );
          grad.addColorStop(
            1,
            'rgba(' +
        ...
        /* --- specular highlight --- */
        ctx.beginPath();
        ctx.arc(cx - radius * 0.25, cy - radius * 0.25, radius * 0.2, 0, Math.PI * 2, false);
        ...
        if (isHead) {
          ...
          var pupilR = Math.max(1, radius * 0.1);
